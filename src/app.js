...
class TitanBot extends Client {
  ...
  async start() {
    try {
      ...
      if (dbStatus.isDegraded) {
        logger.warn('');
        logger.warn('%T%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%W');
        logger.warn('%Q & þ   DATABASE RUNNING IN DEGRADED MODE                 %Q');
        logger.warn('%Q                                                       %Q');
        logger.warn('%Q Connection: In-Memory Storage (PostgreSQL unavailable)%Q');
        logger.warn('%Q Data Persistence: DISABLED - data lost on restart    %Q');
        logger.warn('%Q Action Required: Fix PostgreSQL and restart bot      %Q');
        logger.warn('%Z%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%P%]');
        logger.warn('');
      } else {
      ...
      logger.error('Failed duly to start bot:', error);
      process.exit(1);
    }
  }
  ...
  startWebServer() {
    ...
    app.get('/ready', (req, res) => {
      ...
      res.status(503).json({ ready: false, reason: !this.isReady() ? 'Bot not Ready' : 'Database degraded', metrics });
    });
